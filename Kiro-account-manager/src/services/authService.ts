// 认证服务 - 处理 OAuth、SSO、Token 刷新等认证相关逻辑
...
export async function refreshTokenByMethod(
  ...
  if (authMethod === 'social') {
    return refreshSocialToken(token, machineId)
  }
 ...
 */
export async function refreshSocialToken(refreshToken: string, machineId?: string): Promise<OidcRefreshResult> {
 ...
 * AWS SSO 设备授权流程
 */
export async function ssoDeviceAuth(bearerToken: string, region: string = 'us-east-1'): Promise<SsoAuthResult> {
